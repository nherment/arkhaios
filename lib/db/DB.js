...
//                logger.debug(">> Creating indexes on collection ["+name+"]")
    ...
    this.find = function(collection, query, fields_or_options_or_callback, options_or_callback, callback) {
        ...
        if(collections[collection]) {
            ...
            if(callback) callback(err, undefined)
            else if(options_or_callback) (err, undefined)
            else fields_or_options_or_callback(err, undefined)
    ...
    this.remove = function(collection,selector, optionsOrCallback, callback){
        if(collections[collection]) {
            ...
            var err = new Error("No collection with name ["+collection+"]")
            if(callback) callback(err, undefined)
            else optionsOrCallback(err, undefined)
        }
    }
...
if(!process.env.ARKHAIOS_DB_NAME) {
    throw new Error("Missing env variable [ARKHAIOS_DB_NAME]")
}
if(!process.env.ARKHAIOS_DB_HOST) {
    throw new Error("Missing env variable [ARKHAIOS_DB_HOST]")
}
if(!process.env.ARKHAIOS_DB_PORT) {
    throw new Error("Missing env variable [ARKHAIOS_DB_PORT]")
}
if(!process.env.ARKHAIOS_DB_USER) {
    throw new Error("Missing env variable [ARKHAIOS_DB_USER]")
}
if(!process.env.ARKHAIOS_DB_PWD) {
    throw new Error("Missing env variable [ARKHAIOS_DB_PWD]")
}

logger.info("Connecting to DB with ["+process.env.ARKHAIOS_DB_USER + ":"+process.env.ARKHAIOS_DB_PWD+"@" + process.env.ARKHAIOS_DB_HOST+":"+process.env.ARKHAIOS_DB_PORT+"/"+process.env.ARKHAIOS_DB_NAME+"]")

var dbProvider = new Provider(process.env.ARKHAIOS_DB_NAME,
    process.env.ARKHAIOS_DB_HOST,
    Number(process.env.ARKHAIOS_DB_PORT),
    process.env.ARKHAIOS_DB_USER,
    process.env.ARKHAIOS_DB_PWD,
    function() {

        var runner = new ParallelRunner()

        runner.add(dbProvider.loadCollection, 'images', [
        ...
        runner.add(dbProvider.loadCollection, 'acls', [
            { index: {uid: 1}, options: {unique: true} }
        ])
